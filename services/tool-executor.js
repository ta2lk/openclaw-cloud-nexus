...
function safeResolve(base, userPath) {
  ...
  if (!target.startsWith(base)) throw new Error('⛔ الوصول إلى خارج مساحة العمل غير مسموح');
  return target;
}
...
const tools = {
  ...
  run_command: async (args) => {
    ...
    if (!isAllowed) throw new Error(`⛔ الأمر غير مسموح به: ${command}`);
...
async function executeTool(toolName, toolArgs) {
  ...
  if (!tools[toolName]) return { success: false, error: `الأداة "${toolName}" غير معروفة.` };
  try {
    return await tools[toolName](toolArgs);
  } catch (err) {
